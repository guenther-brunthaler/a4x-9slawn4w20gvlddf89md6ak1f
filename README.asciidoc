...
If keys are not as random, either because the random number generator has biases, or because the key is human-readable text or hexadeciman digits rather than random bytes, then a longer key size can compensate for the reduced entropy per key byte.
...
Actually, the entropy of the hex dump will even be a little bit (although neglectible) higher than that that of the original text, because the mere fact that it *is* a hex dump (just one of many possible encodings) also adds to the amount of information represented by the pass phrase.
...
So use what you feel most comfortable with.
...
base-64 encoding is also very handy, although any "=" characters should be stripped because they are only used for padding and not part of the encoded data themselves. (It does not hurt keeping them, however - it will just make the pass phrase longer without changing its entropy, wasting a few bytes.)
